...
async function main() {
    console.log("Consultando SAP para 80754...");
    const resIn = await sapClient.request(`/IncomingPayments?$filter=DocNum eq 80754`);
    ...
    if (dataIn.value && dataIn.value.length > 0) {
        ...
        const resOut = await sapClient.request(`/VendorPayments?$filter=DocNum eq 80754`);
        ...
        if (dataOut.value && dataOut.value.length > 0) {
            ...
            console.log("No se encontró el documento 80754 en SAP.");
        }
    }
}
