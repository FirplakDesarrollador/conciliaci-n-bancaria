...
async function main() {
    console.log("Consultando SAP para 80832...");
    const resIn = await sapClient.request(`/IncomingPayments?$filter=DocNum eq 80832`);
    ...
    if (dataIn.value && dataIn.value.length > 0) {
        ...
        const resOut = await sapClient.request(`/VendorPayments?$filter=DocNum eq 80832`);
        ...
        if (dataOut.value && dataOut.value.length > 0) {
            ...
            console.log("No se encontró el documento 80832 en SAP.");
        }
    }
}
